...
JSON_INSTRUCTIONS = (
    ...
    '  "risk_reward": {"tp1": число|null, "tp2": число|null, "tp3": число|null},\n'
    ...
    "(точка как разделитель дробной части, без пробелов и разделителей тысяч)."
)
...
SYSTEM_PROMPTS = {
    "scalping": (
        ...
        "уровни поддержки/сопротивления и текущая позиция трейдера, если она есть). Опирайся в "
        "первую очередь на данные младших таймфреймов (1m/5m) — стоп и вход должны быть точными, "
        "в пределах локального шума. Опирайся на конкретные цифры из присланных данных, а не на "
        "общие фразы." + JSON_INSTRUCTIONS
    ),
    "swing": (
        ...
        "индикаторы по нескольким таймфреймам, стакан заявок, funding rate, open interest, "
        "уровни поддержки/сопротивления и текущая позиция трейдера, если она есть). Stop loss "
        ...
        "было не хуже 1:1.5; если ближайшие уровни дают хуже — используй signal=\"WAIT\" вместо "
        "натянутого плана. Опирайся на конкретные цифры из присланных данных, а не на общие "
        "фразы." + JSON_INSTRUCTIONS
    ),
}
